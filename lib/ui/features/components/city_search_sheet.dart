...
class _CitySearchSheetState extends State<CitySearchSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final popular = widget.viewModel.isSearching ? <LocationModel>[] : [
      const LocationModel(name: 'Tokyo', country: 'Japan', countryCode: 'JP', latitude: 35.6762, longitude: 139.6503),
      const LocationModel(name: 'Paris', country: 'France', countryCode: 'FR', latitude: 48.8566, longitude: 2.3522),
      const LocationModel(name: 'New York', country: 'United States', countryCode: 'US', latitude: 40.7128, longitude: -74.0060),
      const LocationModel(name: 'London', country: 'United Kingdom', countryCode: 'GB', latitude: 51.5074, longitude: -0.1278),
      const LocationModel(name: 'Nairobi', country: 'Kenya', countryCode: 'KE', latitude: -1.2921, longitude: 36.8219),
      const LocationModel(name: 'Dubai', country: 'United Arab Emirates', countryCode: 'AE', latitude: 25.2048, longitude: 55.2708),
    ];
                ...
                child: Row(
                  children: [
                    ...
                    if (_controller.text.isNotEmpty)
                      IconButton(
                        icon: const Icon(CupertinoIcons.clear_thick_circled, color: Colors.white54, size: 18),
                        onPressed: () {
                          _controller.clear();
                          widget.viewModel.clearSearch();
                          setState(() {});
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(CupertinoIcons.location_fill, color: Color(0xFF90CAF9), size: 18),
                      ),
                          ...
                          children: [
                            ...
                            Text(
                              widget.viewModel.currentLocation != null
                                  ? widget.viewModel.currentLocation!.displayName
  ...
  Widget _buildSearchResults(List<LocationModel> results) {
    return ListView.separated(
      ...
      separatorBuilder: (context, index) => Divider(color: Colors.white.withValues(alpha: 0.08)),
      itemBuilder: (context, index) {
        ...
        return ListTile(
          ...
          leading: const Icon(CupertinoIcons.placemark, color: Color(0xFF90CAF9)),
          title: Text(
            loc.name,
  ...
  Widget _buildFavoritesAndPopular(List<LocationModel> favorites, List<LocationModel> popular) {
    return ListView(
      physics: const BouncingScrollPhysics(),
      children: [
        if (favorites.isNotEmpty) ...[
          Row(
            children: [
              const Icon(CupertinoIcons.heart_fill, color: Color(0xFFFF5252), size: 14),
          ...
          ...favorites.map((fav) {
            return Container(
              ...
              child: ListTile(
                leading: const Icon(CupertinoIcons.building_2_fill, color: Color(0xFF64B5F6), size: 20),
                title: Text(
                  fav.name,
                  style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600),
                ),
                ...
                trailing: IconButton(
                  icon: const Icon(CupertinoIcons.heart_fill, color: Color(0xFFFF5252), size: 20),
        ...
        Row(
          children: [
            const Icon(CupertinoIcons.globe, color: Color(0xFF81D4FA), size: 14),
            const SizedBox(width: 6),
            Text(
        ...
        Wrap(
          ...
          children: popular.map((city) {
            return ActionChip(
              ...
              avatar: const Icon(CupertinoIcons.placemark, color: Colors.white70, size: 14),
              label: Text(
                city.name,
